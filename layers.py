...
class TransfomerAttentionLayer(torch.nn.Module):
    ...
    def forward(self, b):
        # print("dim_time: ", self.dim_time, " dim_node_feat: ", self.dim_node_feat, " dim_edge_feat: ", self.dim_edge_feat)
        assert(self.dim_time + self.dim_node_feat + self.dim_edge_feat > 0), "At least one of the input features should be non-zero"
        if b.num_edges() == 0:
