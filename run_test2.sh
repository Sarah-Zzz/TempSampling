...
if [ -z "$dataset" ] || [ -z "$config" ] || [ -z "$suffix" ]; then
    ...
    echo "  <config_name> can be: TGN, APAN, JODIE, TGAT"
    echo "  <experiment_type> can be: base, filter, both. Default is 'both'."
    exit 1
fi

# Validate exp argument
allowed_exps=("base" "filter" "both")
...
if [[ "$exp" == "base" || "$exp" == "both" ]]; then
    ...
    date | tee "${base_output_log_file}"
    echo "Command: python train_simple.py --extra_config ${config_file} --logfile ${base_log_file} ${train_simple_args[@]} 2>&1 | tee -a ${base_output_log_file}"
    set -x
...
# --- Filter Run ---
if [[ "$exp" == "filter" || "$exp" == "both" ]]; then
    echo "--- Running with Filter Enabled---"
    psfilter_log_file="${results_dir}/${lower_dataset}_${lower_config}_filter_${suffix}.log"
    psfilter_output_log_file="${results_dir}/output_${lower_dataset}_${lower_config}_filter_${suffix}.log"

    date | tee "${psfilter_output_log_file}"
    echo "Command: python train_simple.py --extra_config ${config_file} --logfile ${psfilter_log_file} --filter ${train_simple_args[@]} 2>&1 | tee -a ${psfilter_output_log_file}"
    set -x
    python train_simple.py --extra_config "${config_file}" --logfile "${psfilter_log_file}" --filter "${train_simple_args[@]}" 2>&1 | tee -a "${psfilter_output_log_file}"
    set +x
    echo "Filter run completed. Logs: ${psfilter_log_file}, ${psfilter_output_log_file}"
